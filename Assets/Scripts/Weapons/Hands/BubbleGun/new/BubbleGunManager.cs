...
public class BubbleGunManager : NetworkBehaviour
{
    ...
    public GameObject ExplosionEffectsPfab;
    ...
    private void OnCollisionEnter(Collision other)
    {
        ...
        if (other.gameObject.layer == 7 || other.gameObject.layer == 9 || other.gameObject.layer == 6 || other.gameObject.layer == 8)
        {
            ...
            GameObject Explosion = Instantiate(ExplosionEffectsPfab, this.transform.position, Quaternion.Euler(ExplosionPoint));
            ...
            if (other.transform.CompareTag("DamageCollider"))
            {
                DM = other.gameObject.GetComponent<DamageManager>();

                if (myTeam == DM._BPM._Team)
                    if (!DM._BPM._WM.FriendlyFire)
                        return;

                Debug.Log("bubbleDamage "+ Damage );
            ...
            StartCoroutine(DestroyBall(Explosion));
    ...
    public IEnumerator DestroyBall(GameObject ex)
    {

        yield return new WaitForSeconds(ImpactTime);
        NetworkServer.Destroy(ex);

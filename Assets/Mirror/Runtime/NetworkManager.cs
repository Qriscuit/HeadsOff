...
namespace Mirror
{
    ...
    public class NetworkManager : MonoBehaviour
    {
     ...
     /// <summary>Multiplayer games should always run in the background so the network doesn't time out.</summary>
        ...
        public void StartClient(string _netAdress)
        {
            if (NetworkClient.active)
            {
                Debug.LogWarning("Client already started.");
            ...
            if (string.IsNullOrEmpty(_netAdress))
            {
                Debug.LogError("Must set the Network Address field in the manager");
                return;
            }
            Debug.Log($"NetworkManager StartClient address:{_netAdress}");

            NetworkClient.Connect(_netAdress);
